...
- **API shape: RATIFIED.** The 12 moved rules stay crate-root `pub fn` in `werust-core` (not a `chrome::` submodule, not methods on `ChromeState`). Ratified deliberately while it was still a rename rather than a migration, before the macOS, Windows and mobile edges bind to it.
- **The stale-class-list bug: TASKED.** Filed as `work/tasks/backlog/export-the-chrome-css-class-set-from-core.md`, to land BEFORE a second painter exists. It exports the complete class set from core, has the painter iterate it, and adds both teeth: exhaustiveness (a new posture without a set entry reds the gate) and no-unstyled-class (an exported name with no `APP_CSS` rule reds the gate).
- **The glossary entry: ADDED.** `CONTEXT.md` now defines "chrome presentation / painter" and contrasts it with "seam", so the next author cannot re-fork the term.
