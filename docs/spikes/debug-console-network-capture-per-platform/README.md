...
6. Load an ENS `.eth` page (the bar shows the pinned name while the scheme tasks carry `ipfs://<cid>/…`): the main-document row must show `name-via-trusted-rpc`, matching the indicator. This is the case Swift's old `chrome().url` compare could never satisfy.
