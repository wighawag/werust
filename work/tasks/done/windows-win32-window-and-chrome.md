...
**The profile folder is YOURS to set, and it is not optional** (planted by the conductor at Gate-3 of the engine task, from a Gate-2 nit). `crates/windows-renderer` defaults its WebView2 user-data folder to `%TEMP%\werust-webview2`, which is right for an engine that only ever runs in a CI smoke and wrong for a browser: a temp profile loses cookies, storage and cache on reboot, and that is a user-visible bug found late and diagnosed slowly. The engine's `DECISIONS.md` states the hand-off but nothing ENFORCES it, so this task's own acceptance criteria now do. Pass a durable path under `%LOCALAPPDATA%`, following whatever the other edges already do for per-user state rather than minting a third convention.
...
- [ ] The shell passes a DURABLE WebView2 user-data folder under `%LOCALAPPDATA%`; it does not inherit the engine's `%TEMP%\werust-webview2` development default.
