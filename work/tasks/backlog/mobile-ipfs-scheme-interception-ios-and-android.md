...
Gate-2 BLOCK (thread safety, must fix): Android runs shouldInterceptRequest on a WebView WORKER thread, but the UI thread independently drives the same CoreSession during an in-flight load (navigate/onPageStarted/onPageFinished + sub-resource interception). The code calls nativeResolveIpfs -> session(handle) (&mut CoreSession) -> resolve_scheme -> inner.borrow_mut() on an Rc<RefCell> (!Sync): two live &mut across threads + a non-atomic RefCell borrow = data race / UB / RefCell panic. Desktop/iOS are sound because their scheme handlers dispatch on the single main/GTK thread; Android alone breaks that. FIX: add a synchronization boundary on the Android edge before touching the core from shouldInterceptRequest — e.g. wrap the session in a Mutex (or marshal the resolve onto a consistent thread / the UI thread), so no two threads hold &mut the CoreSession or borrow the RefCell concurrently. Keep the good work on the branch; this is the one gap to close. Re-verify the mobile ipfs:// interception still works after the sync fix.
