...
Gate-2 BLOCK (parity, must fix): the werust://settings scheme is registered on desktop + Android but NOT on iOS, yet the parity matrix marks retrieval-backend iOS = implemented. On iOS WKWebView will not hand an UNregistered custom scheme to any handler, so register_scheme_handler(WERUST_SCHEME,...) in werust-ios/rust/src/lib.rs is dead — werust://settings is unreachable on iOS. FIX (preferred): register a WKURLSchemeHandler for the 'werust' scheme in crates/werust-ios/App/Sources/WKWebViewShellController.swift (mirror the setURLSchemeHandler forURLScheme:'ipfs' you already added for the ipfs interception task), and add the FFI/Swift routing so a werust://settings request reaches apply_settings_request (mirror werust_ios_resolve_ipfs -> add werust_ios_apply_settings or equivalent). Keep the desktop/Android work. If for some reason iOS truly cannot host it this round, instead flip the retrieval-backend iOS matrix cell to stubbed with a linked follow-on task (do NOT claim implemented while unreachable) — but prefer actually wiring it, the ipfs task proved the mechanism. Re-verify the setting switches the ContentRetriever on all three platforms.
