...
**Why.** The warning half is pure derivation and reaches the AppKit window through the shared `desktop_paint::ChromePaint` with no macOS change at all. The bless half needs a click target, and the AppKit trust indicator is a plain `NSTextField` label. Android and iOS each already had an alert to extend (the explanation alert `mobile-chrome-presentation-from-one-derivation` added); GTK did NOT, and its surface was BUILT here, by turning the badge from a hover-tooltip `Label` into a `MenuButton` opening a `Popover` (`crates/werust/src/main.rs`). So macOS is the one edge left without a surface, and that is a missing WIRE, not a missing platform, so `n-a` would be a lie. Splitting the rows is the precedent `trust-explanation` set when it was split out of `trust-indicator` "precisely so that gap can never hide inside a row that is green for the badge".
...
**Corrected 2026-07-31** by `pin-store-read-modify-write-and-test-isolation`. As written at Gate-3, the **Why** above claimed GTK "already had a popover" to extend. That is not what the diff did: the GTK trust surface is NEW in this task, which turned the badge from a hover-tooltip `Label` into a `MenuButton` + `Popover`. The false history mattered because the next agent reads it as the reason macOS may be the only edge that has to BUILD a surface, and would take "everyone else already had one" as permission to skip building one elsewhere. The same claim was corrected in the `macos-trust-surface-bless-affordance` task body and in `docs/platform-capability-matrix.toml` (the `trust-explanation` and `mutable-name-tofu-bless` rows).
