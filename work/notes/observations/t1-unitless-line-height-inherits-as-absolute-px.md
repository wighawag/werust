...
2026-07-22 (resolved) — Fixed by task `fix-unitless-line-height-inherits-as-multiplier`. `ComputedStyle.line_height` is now a `css::LineHeight` enum (`Normal` / `Absolute(px)` / `Multiplier(n)`): a unitless value inherits as the multiplier and is re-resolved against each element's own font-size at use (shaping + the WPT meter), while unit-bearing and `normal` keep their behaviour. The WPT meter case `line-height-unitless-inherits-as-multiplier` now PASSES (core-CSS 27/27).
