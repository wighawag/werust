...
**No longer in scope (2026-08-17, `to-task` on spec `trust-store-hardening`):** this task originally asked for the cross-process non-atomicity of read-modify-write to be RECORDED as a known limit. It is now being FIXED instead, by `trust-store-serialises-read-modify-write-so-no-bless-is-lost`, which holds an advisory lock across the read and the save. Do not record it as a standing limit here: a note saying "this cannot be fixed cheaply" beside a task that fixes it is exactly the contradiction the tree should not carry.

**Sequencing (read this before scheduling it).** This task should land BEFORE spec `trust-store-hardening`'s six-task chain, whose head is `trust-store-fails-closed-instead-of-reading-as-nothing-trusted`. That chain rewrites the store's load surface (an unreadable store becomes a distinct third state, not an empty one), which is precisely the shape items 1 and 2 below describe. Landing this first gives that chain ONE well-defined read site to teach about the third state; landing it second turns this task into a drifted one that needs re-deciding rather than building.

**Scope:** one refresh trigger with its test, one duplicate loader removed, the mobile test isolation, and one recorded note. No change to the TOFU model, the trust posture, or the banner rules.
...
Attempt 2 died on upstream model capacity (overloaded_error), before any build work: nothing about the repo, the gate or your code. Continue from the kept branch tip.
