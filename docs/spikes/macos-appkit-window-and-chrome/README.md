...
- **`crates/werust-macos`** — the AppKit window, in two halves:
  - the **host-independent** half, `src/paint.rs` as this task landed it and today [`crates/desktop-paint`](../../../crates/desktop-paint) (EXTRACTED verbatim, with its tests, by task `windows-win32-window-and-chrome` so the Win32 window consumes the one carrier; `werust_macos::paint` re-exports it, so nothing about this window changed): every value the window paints, derived from the shared `werust-core` rules (`status_line`, `trust_indicator` / `_detail` / `_css_class`, `error_banner_*`, `invalid_entry_badge_*`, `load_progress_*`, the debug view's row rules, the `BrowserMenu` items) plus this edge's own palette and the macOS debug CAPTURE POINTS over the `Renderer` seam. It compiles and is **unit-tested on Ubuntu against the real core**, so "the macOS window paints the shared derivation" is a checked fact, not a claim.
