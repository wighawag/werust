...
mod tests {
    ...
    // The pure `LoadLifecycle` STATE-MACHINE tests that used to sit here MOVED to
    // `crates/webview-shared/src/lifecycle.rs`, beside the code they cover, when
    // task `macos-spike-doc-accuracy-and-harness-guard` found the shared crate
    // shipping its central guarantee untested anywhere but this GTK-bound crate
    // (so `cargo test -p webview-shared` on the macOS runner exercised none of
    // it). What stays here is what is about THIS backend: the seam contract driven
    // through `SeamHarness`, the trust-hook wiring, and the GTK-specific rules.
