...
- **Kotlin at the OS edge only.** `BrowserActivity` (a plain framework `Activity`) hosts a URL bar, Back/Forward/Reload/Stop buttons, and the platform `android.webkit.WebView`. It holds NO browsing logic: every decision (URL-bar text, Back/Forward availability, load status) is the Rust core's truth, read back through `WerustCore`.
