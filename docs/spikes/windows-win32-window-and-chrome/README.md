...
Not in scope, deliberately: no installer, no code signing, no zip on a Release (task `windows-release-packaging-leg`, which has since landed the zip and the application manifest — [its record](../windows-release-packaging-leg/README.md)), and no `windows` column in the platform-capability matrix (task `windows-parity-column-and-stub-tasks`, which runs after this so the cells describe what really shipped). Both were authored by this task.
...
- **The chrome's STYLE is now modern, and nobody has looked at it.** ~~Without a comctl32 v6 manifest the controls draw in the pre-Vista style~~ — **UPDATED 2026-07-31 by `windows-release-packaging-leg`**, which embedded the manifest ([its decisions](../windows-release-packaging-leg/README.md)). The comctl32 v6 dependency is now in every binary this crate links, so the controls should draw in the current visual style. "Should": no runner can see a pixel, and this list's whole point is not to claim what nobody has looked at. **The dark-mode half of that old line was WRONG and is not closed by the manifest**: a v6 dependency buys visual styles, not dark mode, and standard Win32 controls follow the OS light/dark setting only through an undocumented uxtheme path — so the chrome's push BUTTONs are still light on a dark-mode Windows. Sources: `work/notes/findings/win32-common-controls-dark-mode-needs-more-than-a-v6-manifest-2026-07-31.md`; the work is now `work/tasks/backlog/windows-chrome-dark-mode-for-common-controls.md`, which is where the `follow-os-color-scheme` parity cell points.
- **HiDPI: the process is now DPI-AWARE, the chrome's LAYOUT is not.** ~~There is no per-monitor-v2 manifest entry, so Windows will bitmap-scale the chrome~~ — **UPDATED 2026-07-31 by the same task**: the manifest declares `PerMonitorV2` (with the pre-1607 `dpiAware` fallback), so Windows no longer bitmap-scales the process and the PAGE should render at the real device scale. But every rectangle in `chrome.rs` is still a raw-pixel constant and `ui_font()` still hardcodes a 96-DPI height, and there is no `WM_DPICHANGED` handler, so on a 150%/200% display expect the chrome to be crisp but SMALL, and expect nothing in particular when dragging between monitors of different scale factors. That is a different failure mode from the blur this line used to predict, and it is still unmeasured — see manual step 10, and `work/notes/observations/windows-chrome-is-raw-pixels-under-per-monitor-v2-2026-07-31.md` for the follow-on's shape.
...
- **Unsigned.** `windows-release-packaging-leg` now attaches `werust-windows-x86_64-unsigned.zip` to a tagged Release, but nothing is SIGNED and nothing is installed: SmartScreen's actual behaviour on a fresh machine, and upgrade/uninstall (which do not exist — it is a zip), remain untouched.
...
10. **HiDPI.** Open it on a 150% or 200% display, and drag it between monitors with different scale factors. Since the per-monitor-v2 manifest landed, expect the PAGE to be crisp and correctly scaled and the CHROME to be crisp but under-sized (the toolbar, the badges and the text are laid out in raw 96-DPI pixels), and expect the chrome not to re-lay-out at all when the window crosses to a monitor with a different scale factor. Record what you actually see, that is the input the DPI follow-on needs.
11. **Visual styles.** The buttons, the URL bar, the tabs and the list views should look like the rest of Windows 11, not like Windows 2000 — that is what the comctl32 v6 manifest is for, and it has never been looked at. Two specific things to check while you are there: the URL bar's in-flight progress strip must still be werust's BLUE (the shared palette's, kept by opting that one control out of theming) and not the system's green; and in dark mode the push buttons will still be LIGHT, which is a known, tracked gap, not something you found.
12. **Resize, maximise, minimise, restore.** The page keeps filling the area between the toolbar and the status line at every size.
13. **A `target="_blank"` link navigates IN PLACE** (ADR-0010) — no second window opens.
14. **The profile is durable.** Confirm `%LOCALAPPDATA%\werust\WebView2` exists after a run, and that a cookie/localStorage value survives closing and re-opening werust.
