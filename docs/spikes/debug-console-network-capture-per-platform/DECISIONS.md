...
- **The posture read is LIVE, never the cached chrome snapshot** (the round-2 Gate-2 fix). `ChromeState.trust_posture` is only re-cached by `refresh_chrome`, which runs on the page commit/finish signals — AFTER the scheme handler has resolved, marked the backend verified, and the capture has fired. Reading the cache there stamped the stale pre-verify `unverified-origin` on exactly the ENS/`ipfs://` main-document row the reconciliation exists for, DOWNGRADING it below the honest per-request posture it would otherwise have carried. So the mobile capture points read `BrowserShell::live_trust_posture` (the seam's `Renderer::trust_posture`, which the mobile backends implement over their live per-load posture), the same fact the desktop capture reads directly from its load lifecycle (`self.life.borrow().posture()`). Pinned by the ordering-trap tests: a main-document capture BEFORE any `refresh_chrome` still carries the live posture.
