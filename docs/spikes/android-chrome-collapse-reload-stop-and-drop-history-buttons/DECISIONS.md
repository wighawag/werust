...
> SINCE SUPERSEDED (2026-08-04): the fan-in task landed the registration and INVERTED this assertion into `the_mobile_presentation_guard_registers_the_fields_this_edge_consumes`, which now holds the central guard to keeping the entries. See `docs/spikes/register-the-new-chrome-fields-in-the-mobile-presentation-guard/DECISIONS.md` §2. The reasoning below is the record of why the hold existed, not the current shape.

**Why:** the guard demands that BOTH mobile bindings decode and BOTH painters paint every registered derived field. This is the MIGRATE step for ONE edge, so registering the fields here would red the gate until `ios-chrome-collapse-reload-stop-and-drop-history-buttons` lands. The CONTRACT step is `register-the-new-chrome-fields-in-the-mobile-presentation-guard`, blocked on both edges. Asserting the absence (rather than just leaving it alone) is what surfaces a well-meaning early registration as a NAMED failure pointing at the fan-in task, instead of as an opaque "the iOS binding does not decode this field" red that invites "fixing" by weakening the shared guard, which is the one mechanism keeping the Kotlin/Swift chrome twins from coming back.

**How the assertion is made honest (a review finding, recorded so it is not re-broken):** the first version of this assertion scanned the guard and asserted the field was absent from the scanner's CODE view. That could never fail, because the scanner strips string literals out of the code view, and a field name can only ever appear in that guard AS a string literal (a `DERIVED_FIELDS` entry, or an FFI signature string). It now asserts against the LITERAL half, and carries a POSITIVE CONTROL: an already-registered field (`loadProgressVisible`) must be visible to the same check, at both ends of the file. Mutation-checked: adding `"loadSpinnerVisible"` to `DERIVED_FIELDS` turns it red, naming the fan-in task.
