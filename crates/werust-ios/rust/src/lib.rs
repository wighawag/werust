...
impl CoreSession {
    ...
    /// the progress fraction + hint), so the Swift edge reads a field instead of
    /// re-deriving the rule (task
    /// `mobile-chrome-presentation-from-one-derivation`). It used to be encoded by
    /// a local `ffi_json` module that was a byte-for-byte twin of the Android
    /// one: the same duplication one level down.
    #[must_use]
