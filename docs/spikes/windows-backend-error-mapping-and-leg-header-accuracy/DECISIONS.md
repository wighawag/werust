...
**Chosen:** a new pure function `windows_renderer::pure::environment_creation_error(detail)` alongside the existing `missing_runtime_error(detail)`. Both are plain `RendererError::Backend`s. The new one names the runtime as INSTALLED, says the environment was refused, and KEEPS the platform's own text verbatim after a colon, ending the sentence with it (this line said "LEADS with" until `windows-release-packaging-leg` corrected it, together with the rustdoc and the test message that said the same thing: the detail trails the sentence, it does not open it — what matters is that it is kept intact rather than buried in a parenthetical); it never mentions the download. `create_environment`'s two failure paths now map through it, and `missing_runtime_error` is left with the one call site where the runtime may genuinely be absent: `runtime_version()`, the `GetAvailableCoreWebView2BrowserVersionString` presence check.
