...
| the 50ms pump in `open_window` | refreshes the open view on the EXISTING cadence (incremental append; no new timer, no busy loop) |
...
- `crates/werust/src/main.rs` unit tests: the render-from-store mapping (console row text with level + message + source:line and honest absent fields; the per-level CSS classes all distinct; network status/mime/size columns with unknowns as `?`; the trust column composed of the indicator's glyph + the core's wire name + the indicator's own CSS classes, distinct per posture, including the spec's ipfs:// = `✓ content-verified` / https:// = `⚠ unverified-origin` split).
