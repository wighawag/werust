...
> Tasked 2026-08-16 (`to-task`): the Implementation / Testing detail this spec carried moved INTO the tasks it emitted (`work/tasks/`, carrying `spec: android-instrumented-ci-leg`), which are the current truth for what to build. Durable rationale is recorded as ADRs by the tasks that decide it, not predicted here.
