...
iOS could NOT be run on this host (Linux; the iOS shell needs Xcode/a simulator), so this is a mechanism analysis with the runtime confirmation left as recorded steps, not a device run. **Read the 2026-07-30 addendum below with it:** the WebKit half of what follows has since been MEASURED on a real macOS runner, and the analysis held.
...
### Addendum 2026-07-30 — the mechanism is now MEASURED, on a real WebKit (`macos-wkwebview-renderer-backend`)

The paragraph above was, as written, a mechanism analysis. It is no longer only reasoned about. The macOS WKWebView backend task built `crates/macos-origin-probe`, the WebKit analogue of `crates/windows-origin-probe`, which loads a canned SvelteKit-shaped page from `ipfs://<cid>/` through a registered `WKURLSchemeHandler` and measures the four facts this caveat rests on, against a negative control (the identical bytes with an opaque origin) that must reproduce the Android failure. **That probe has now RUN**, on a `macos-14` runner, macOS 14.8.7 (Build 23J520), AppleWebKit/605.1.15: [run 30563185521](https://github.com/wighawag/werust/actions/runs/30563185521), verbatim output committed at `docs/spikes/macos-wkwebview-renderer-backend/probe-report-2026-07-30.json`.

**What it measured**, on the handler-served `ipfs://<cid>/` document: origin `ipfs://bafybei…pfzq` (a real tuple origin, NOT opaque), the same-origin `fetch('/blog/__data.json?x-sveltekit-invalidated=01')` `ok:200` **and** the scheme handler fired for it, and `history.pushState({}, '', '/blog/')` `ok:/blog/`. The negative control, on the same runner in the same process with the same handler still installed, reproduced the Android failure shape exactly: origin `null`, `fetch` `reject:TypeError` with the handler never fired, `pushState` `throw:SecurityError`. So the difference is WebKit's behaviour, not a harness that cannot fail.

What that does and does not settle, stated precisely:

* `WKURLSchemeHandler` is ONE WebKit class. What the run measured about the origin a handler-served document receives is a property of the WebKit port, which macOS and iOS share, so **the load-bearing half of this caveat is settled by measurement for both**: the analysis above was right, and iOS does not share Android's cause.
* It did NOT build the iOS app, load `ronan.eth`, or click a blog link. The device-level steps recorded above stay open, and the residual risk they cover (something iOS-specific in `WKWebViewShellController`'s wiring rather than in WebKit's origin model) is untouched by this run.
* The measurement is from a CI runner, not from Mac hardware in front of a human. That is sufficient for an origin question and insufficient for anything about rendering, input or focus. See `docs/spikes/macos-wkwebview-renderer-backend/README.md` ("The MEASURED verdict" / "What CI proved" / "What still awaits a Mac") for the full split, including the one field that moved against the prediction (`isSecureContext` measured TRUE, which decides nothing here).
