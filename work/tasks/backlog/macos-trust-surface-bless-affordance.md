...
The blocker is the widget, not the platform: the AppKit trust indicator is a plain `NSTextField` label (`crates/werust-macos/src/window.rs`), and a label has no click target. Android and iOS only had to add a line and a button to a surface they already had (an `AlertDialog` / a `UIAlertController`, both added by `mobile-chrome-presentation-from-one-derivation`). GTK did NOT have one either: its badge was a plain `Label` with a hover tooltip, and `ipns-tofu-pin-and-warn-on-change` BUILT the desktop surface by turning it into a `MenuButton` opening a `Popover` (`crates/werust/src/main.rs`) — so GTK is the precedent for what this task must do, not an example of an edge that got it for free. macOS needs that same surface built: turn the badge into something clickable (an `NSButton`, or a click gesture recogniser on the field) and open a small panel/popover carrying, in this order, the core's `trust_indicator_detail`, the core's `trust_pin_detail`, and (shown exactly when the core's `trust_pin_action_visible` is true) a button whose title is the core's `trust_pin_action_label`, activating `BrowserShell::bless_current_name`.
...
Windows is in the same position for the same reason and is NOT covered here. **DRIFT UPDATE (conductor, 2026-07-31): the `windows` column now EXISTS** — `windows-parity-column-and-stub-tasks` landed, and it cut `windows-trust-surface-bless-affordance` as the Win32 twin of this task, with its own `stubbed` cell pointing at it. So the reason to leave the Win32 window alone is no longer "its column does not exist"; it is simply that a sibling task owns it. Stay out of the Win32 window, and if you build anything the two windows will both want, put it in `crates/desktop-paint` so the sibling consumes it rather than re-deriving it.
