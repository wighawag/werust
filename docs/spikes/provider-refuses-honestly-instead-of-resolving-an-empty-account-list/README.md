...
- **`crates/werust-core/src/ethereum.rs` — one chain constant, in the module that owns the chain.** `STUB_CHAIN_ID` is gone. `ethereum::CHAIN_ID` is now the ONE place werust states which chain it is on, and `provider::CHAIN_ID` is a `pub use` re-export of it (one value, two paths, no second constant to drift). Both trust-hook smokes compare against `werust_core::provider::CHAIN_ID`, so they follow the rename with no behaviour change.
