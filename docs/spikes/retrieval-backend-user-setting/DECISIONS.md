...
## Requeue fix (2026-07-23) — iOS `werust://settings` was unreachable (Gate-2 parity BLOCK)

**The gap (as reviewed):** the first attempt registered the `werust` scheme handler on the Rust side of ALL three backends (`install_ipfs` calls `register_scheme_handler(WERUST_SCHEME, apply_settings_request)`), and the parity matrix marked `retrieval-backend ios = implemented`. But on iOS a `WKWebView` will NOT hand an UNregistered custom scheme to any handler, and the Swift shell only ever registered `setURLSchemeHandler(…, forURLScheme: "ipfs")` — never `werust`. So the Rust-side `werust` handler was DEAD on iOS: `werust://settings` was unreachable, the criterion-5 (present on iOS) and criterion-2 (load-path switch reachable via the UI) claims were false there. Desktop (WebKitGTK `register_uri_scheme`) and Android (the generic `WerustCore.resolveIpfs` → `resolve_scheme` dispatch through `shouldInterceptRequest`, which routes ANY registered scheme) were genuinely wired.

**Chosen fix (preferred path, wire iOS — not flip to stubbed):** mirror the `ipfs` mechanism the ipfs-interception task proved:

- Rust core (`crates/werust-ios/rust/src/lib.rs`): add `CoreSession::apply_settings(uri)` (the twin of `resolve_ipfs`, both going through the generic `IosHandle::resolve_scheme` dispatch) and export it over the C-ABI as `werust_ios_apply_settings` (declared in `crates/werust-ios/Sources/werust_mobile.h`). Kept as a DISTINCT edge method + FFI export (rather than reusing the misnamed `resolve_ipfs`) so the Swift shell registers a `WKURLSchemeHandler` per scheme and each is honestly named for what it serves.
- Swift shell (`crates/werust-ios/App/Sources/`): `WerustCore.applySettings(uri)` wraps the new FFI (mirroring `resolveIpfs`), and `WKWebViewShellController` registers `configuration.setURLSchemeHandler(WerustSchemeHandler(core:), forURLScheme: "werust")` next to the existing `ipfs` registration, with a `WerustSchemeHandler: WKURLSchemeHandler` mirroring `IpfsSchemeHandler` that serves the page bytes / fails closed on a bad host.

So `werust://settings` is now reachable on iOS through the SAME shared-core `apply_settings_request` path desktop + Android use, the selection persists to the same isolated settings file, and it switches the `ipfs://` load path on the next session on all three platforms.

**Alternative considered (rejected this round):** flip the iOS matrix cell to `stubbed` with a linked follow-on. Rejected because the ipfs task already proved the exact mechanism (an explicit per-scheme iOS `setURLSchemeHandler`), so actually wiring it was the small, certain move; claiming `implemented` while unreachable was the original defect.

**Touches:** the parity matrix `retrieval-backend` row comment (`docs/platform-capability-matrix.toml`) now spells out the per-scheme iOS registration so the `ios = implemented` cell is truthful. The Swift edge is not compiled by the workspace `cargo`/`verify` gate (it needs Xcode/simulator), so the Rust FFI + bridging-header + a C-ABI test (`the_c_abi_applies_settings_through_the_core_and_frees_its_handle`) are what the gate covers; the Swift `WerustSchemeHandler` mirrors the already-shipping `IpfsSchemeHandler` structure exactly.
