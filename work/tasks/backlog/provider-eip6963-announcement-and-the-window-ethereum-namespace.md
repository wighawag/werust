...
**Two files outside the core move with this decision, and one of them is moving house (noted 2026-08-16, `to-task` on spec `werust-test-pages`).** The provider mimic (`werust-provider-mimic.js`) is a faithful port of `crates/werust-core/src/provider.rs`'s shim and answer path, and the provider inspector page reports EIP-6963 enumeration as one of its results, so any change to what werust announces must update BOTH or they start lying about werust to whoever uses them to test a dapp. Task `publish-the-provider-inspector-from-the-website` MOVES both files from `docs/dev/` to `website/test/` (unchanged apart from path fixes), leaving a pointer in `docs/dev/README.md`. Check where they live before you edit them, and treat updating them as part of this task rather than a follow-on.
