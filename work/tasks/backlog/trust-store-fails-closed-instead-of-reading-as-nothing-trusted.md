...
Carry the third state through EVERY shell-side read, so no caller can flatten it back into "empty" by accident. (Correction, 2026-08-17: this task was written when the shell read the store once, at construction. `pin-warning-reads-a-stale-cache-so-another-windows-bless-never-warns` has since landed, so there are now several sites going through the shell's pin-store location: construction, the two explicit-location builders, the bless read-modify-write, and the cache refresh at every navigation. They all funnel through one location type, which is where the third state belongs; the sites that currently write `unwrap_or_default()` are exactly the ones that would flatten it.) the shell reads the store once per load and the chrome's TOFU axis is derived from that read, so the shell must decide, explicitly, what the chrome shows and what a bless attempt does while trust cannot be determined. Note the shell's location type already distinguishes "there is nowhere to read from" (its `None`) from "an empty store"; the third state is a THIRD answer beside those two, not a re-use of either. Whatever it does, it must not claim a name is unblessed (which would offer the user a bless that would be REFUSED by the write rule) and must not break browsing: a store werust cannot read is not a reason to fail a load.
