...
| `tail_plan` | the pure per-tab refresh plan: append the entries after the last-rendered sequence, dropping from the view's top the rows the ring buffer evicted from the store's front; rebuild when that anchor was evicted (at the ring-buffer cap) or the store was cleared |
...
- `crates/werust/src/main.rs` unit tests: the render-from-store mapping (console row text with level + message + source:line and honest absent fields; the per-level CSS classes all distinct; network status/mime/size columns with unknowns as `?`; the trust column composed of the indicator's glyph + the core's wire name + the indicator's own CSS classes, distinct per posture, including the spec's ipfs:// = `✓ content-verified` / https:// = `⚠ unverified-origin` split), and the refresh plan (`tail_plan`): the past-cap test drives the REAL store past its 300-entry cap one push per tick and asserts the newest entry still renders, the row count STAYS AT the cap across the incremental at-cap appends, and the view's rows mirror the store exactly (its top rows are never ones the store evicted) (the Gate-2 defect, both rounds), display-free and network-isolated.
