...
- **Desktop (WebKitGTK):** `WebViewRenderer::install_new_window_in_place` wires the `create` signal, reads the navigation action's target URI, loads it into the existing `self.view` via `load_uri`, and returns **NULL** so WebKitGTK creates no second WebView. It wires `create` through the RAW glib signal API (`connect_local("create", …)`, answering `None::<gtk4::Widget>`) rather than the typed `connect_create` binding, whose generated return type is a NON-nullable `gtk::Widget` and so cannot express the "no new view" answer the signal's contract requires. `create` is the one hook that covers BOTH triggers: `window.open(url)` reaches `create` directly (`LocalDOMWindow::open` -> `WebCore::createWindow` -> `Chrome::createWindow`) and never emits a `decide-policy` `NewWindowAction`, so `decide-policy` alone would handle only the `target="_blank"` link.
...
- CORRECTION (2026-07-26, task `fix-desktop-create-signal-crash-on-blank-links`): the desktop hook as FIRST implemented returned the EXISTING view from `create` instead of NULL, because the typed `connect_create` binding has no nullable return. That answer is neither a new view nor NULL, so WebKitGTK applied the pending window's `WindowFeatures` to the returned view, dereferenced an empty `std::optional<WebCore::WindowFeatures>`, and ABORTED (SIGABRT, `_M_is_engaged()` failed) on every `_blank` click and every `window.open` in the shipped v0.2.5 desktop build. The DECISION recorded here (in-place until tabs exist) is unchanged; only the desktop MECHANISM was corrected to the raw-signal-returns-NULL shape described above. Diagnosis, the rejected `decide-policy` route, and the measured before/after on `ronan.eth`: `docs/spikes/fix-desktop-create-signal-crash-on-blank-links/README.md`.
