...
**The debug-view presentation may already be shared by the time you start.** `desktop-chrome-presentation-into-core` moved the CHROME rules into core but left the DEBUG-VIEW row helpers (`console_level_css_class`, `console_source_line`, `console_row_text`, `network_status_text` / `_mime_text` / `_size_text` / `_trust_label` / `_trust_css_class`) private in the GTK edge; `macos-appkit-window-and-chrome` owns extracting them. If that landed first, CONSUME the shared versions. If it did not, extract them here the same way (behaviour-preserving, tests moving with them) rather than re-deriving them in the Windows edge.

**Consider splitting this task the way macOS was split** (engine first, then the window that paints), for the same reason: the `Renderer` backend can land and be reviewed on its own, and the chrome work is where the bulk of the days go.
