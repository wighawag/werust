...
MECHANISM CORRECTION (the agent's STOP was right): do NOT use decide-policy as the sole route — decide-policy/NewWindowAction fires for target=_blank but NOT for window.open(), so decide-policy-only would SILENTLY REGRESS window.open on desktop (which iOS/Android native hooks DO handle), breaking the ADR-0010 parity claim. Use OPTION (b): handle the 'create' signal via the RAW glib signal connect_local("create", ...) returning a NULL Widget (glib None::<gtk4::Widget> / the documented 'no new view' answer). The typed webkit6 connect_create wrapper's NON-nullable gtk::Widget return is exactly what forced the crashing return-the-existing-view shape; the raw connect_local lets you return NULL. In the raw handler: extract the NavigationAction arg from the signal values, read its request URI, apply the shared renderer::new_window_action rule, on NavigateInPlace{url} do self.view.load_uri(&url) + life.begin(&url) (so ipfs:// stays hash-verified via install_ipfs and unsupported schemes are refused — no trust bypass, bar follows the URL), and return NULL so WebKitGTK creates NO new view (no WindowFeatures deref, no SIGABRT). This keeps BOTH target=_blank AND window.open in place through ONE hook with no capability regression. ACCEPTANCE additions: (1) BOTH a target=_blank link AND a window.open(url) load in place without aborting; (2) UPDATE docs/adr/0010, the blank-window-open matrix row in docs/platform-capability-matrix.toml, and docs/spikes/blank-and-window-open-links-navigate-in-place/README.md — they currently all describe the crashing 'returns the EXISTING view widget' shape and must be corrected to the raw-create-returns-NULL mechanism. Keep a code comment explaining WHY the typed connect_create cannot be used (non-nullable Widget return). iOS/Android unchanged.
