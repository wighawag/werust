...
CONDUCTOR HANDOFF (2026-07-30, drive-tasks). Gate 2 blocked this correctly: the macos-14 leg was EXTENDED but never DISPATCHED, so no line of the AppKit half had runtime evidence. That was not something you could fix from inside the build: your work branch only reaches the arbiter when the run surfaces, so you cannot dispatch a workflow against it mid-build. The conductor did it for you.

THE LEG IS GREEN AGAINST THIS BRANCH: run 30572253620 (workflow_dispatch, macos-renderer, ref work/task-macos-appkit-window-and-chrome). ALL steps succeeded, including step 5 "Build the macOS backend, the window and the origin probe", step 6 "Test the macOS crates and the shared toolkit-free half", step 7 the trust hooks on a real WKWebView, step 8 "Build and drive the REAL AppKit window (chrome, menu, debug view)", and step 9 the origin measurement.

window_smoke output, verbatim, ending in PASS:
  the window opens with the core's own default chrome: trust indicator shows the core's badge; the trust badge carries the core's EXPLANATION as its tooltip; the status line shows the core's status; no error banner; no invalid-entry badge.
  the menu is the shared core's BrowserMenu, item for item: the NSMenu's titles are the core's item labels, in order.
  a hash-VERIFIED ipfs:// page (offline, pinned): the verified load settles; the URL bar shows the loaded content-addressed URL; the trust indicator paints the core's verdict; a hash-verified page reads as verified; the status line still mirrors the core; a successful load raises NO banner (progress lived in the URL bar); the page view did NOT move or resize across a whole load.
  the debug view, opened through the core menu's Debug entry: the Debug item is enabled and activatable; the page's console.log reaches the Console tab; the Debug entry opened the view; clearing the shared store empties both tabs.
  the NEGATIVE CONTROL (bytes that do not hash to their CID): the tampered load FAILS; a failed load raises the prominent error banner carrying the core's protocol-named reason ("block hash mismatch: bytes do not match cid bafkrei..."); a FAILURE is the one state allowed to displace the page; a failed load is never reported verified.
  closing the debug window drops the slot, so Debug opens a fresh one.

WHAT TO DO: record this run as evidence and correct the honesty sections. Specifically: (1) replace the spike README's "What CI proved: Nothing about this window, yet" with what the run actually proved, naming the run id and the OS/WebKit build the leg records; (2) fix DECISIONS.md 11, which repeats the not-yet-run claim; (3) keep anything the leg did NOT exercise listed honestly as awaiting hardware, and be specific about which assertions came from window_smoke versus from host-independent tests. Do NOT restate the whole transcript as prose; cite it and keep it accurate. Change no behaviour: the code is what passed.
